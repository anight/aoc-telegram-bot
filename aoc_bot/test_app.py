...
from aoc_bot import bot, models, reports

cfg = bot.Settings()

TEST_RESOURCES = "aoc_bot/test_resources"

# Unit tests


def test_parse_aoc_json_response():
    with open(f"{TEST_RESOURCES}/board_2.json") as f:
...
@pytest.mark.parametrize(
    "current_board_file,fetched_board_file,expected_report_file",
    [
...
def test_run_once(current_board_file, fetched_board_file, expected_report_file):
    ...
    with patch("aoc_bot.bot.aoc_fetch_api") as fetch_mock, patch(
        "aoc_bot.bot.send_telegram_message"
    ) as send_mock:
        ...
        for chat in local_cfg.telegram_chats:
            send_mock.assert_called_with(
                local_cfg.telegram_token, chat, expected_report
            )


@pytest.mark.parametrize(
    "board_file,report_file",
    [
...
@pytest.mark.parametrize(
    "board_file,board_file_new,report_file",
    [
        ("board_1.json", "board_2.json", "board_2-1.report"),
        ("board_2.json", "board_3.json", "board_3-2.report"),
    ],
)
def test_text_leaderboard_diff(board_file, board_file_new, report_file):
    with open(f"{TEST_RESOURCES}/{board_file}") as f:
        lb = models.LeaderBoard(**json.load(f))
...
def test_notify_telegram_chats():
    with open(f"{TEST_RESOURCES}/board_1.json") as f, open(
        f"{TEST_RESOURCES}/board_2.json"
    ) as f2:
